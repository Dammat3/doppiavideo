...
app = FastAPI(title="DoppiaVideo API")
...
@app.get("/health")
async def health():
    return {"status": "ok", "whisper_model": WHISPER_MODEL, "max_file_mb": MAX_FILE_SIZE_MB, "max_duration_min": MAX_DURATION_SEC // 60}
...
@app.get("/jobs")
async def list_jobs():
    """Ritorna tutti i job con status, per la coda."""
...
async def dub_video(
    ...
    if not file.filename.lower().endswith(allowed_ext):
        raise HTTPException(400, f"Formato non supportato. Usa: {', '.join(allowed_ext).upper()}")
    ...
    if duration > MAX_DURATION_SEC:
        shutil.rmtree(job_dir, ignore_errors=True)
        raise HTTPException(400, f"Video troppo lungo ({int(duration//60)}m {int(duration%60)}s). Limite: {MAX_DURATION_SEC//60} minuti.")

    # Conta quanti job sono attivi (processing/queued) per la posizione in coda
...
async def download(job_id: str):
    ...
    return FileResponse(output_path, media_type="video/mp4", filename=f"doppiato_it_{job_id[:8]}.mp4")
...
async def process_video(job_id: str, video_path: Path, voice: str, source_lang: str):
    # Aspetta se c'è già un job in processing (coda sequenziale)
    while True:
        processing = [jid for jid, j in jobs.items() if j["status"] == "processing" and jid != job_id]
        if not processing:
            break
    ...
    # Risolvi la voce esplicitamente dal dizionario
    voice_name = VOICES.get(voice, VOICES["isabella"])

    try:
        # Step 1 — Estrai audio
        update_job(job_id, step=1, step_name="Estrazione audio...", progress=5)
        ...
        if result.returncode != 0:
            raise Exception(f"FFmpeg estrazione audio fallita: {result.stderr.decode()}")

        # Step 2 — Trascrivi con Whisper (alta qualità: beam_size=10, best_of=5)
        update_job(job_id, step=2, step_name="Trascrizione (Whisper AI)...", progress=20)
        ...
        # Step 3 — Traduci con DeepL (fallback su Google se DeepL non disponibile)
        update_job(job_id, step=3, step_name="Traduzione in italiano (DeepL)...", progress=50)
        translation = translate_best(transcript, detected_lang)
        ...
        # Step 4 — Sintesi vocale con la voce corretta
        update_job(job_id, step=4, step_name=f"Sintesi voce italiana ({voice})...", progress=65)
        tts_path = job_dir / "tts_audio.mp3"

        communicate = edge_tts.Communicate(translation, voice_name)
        await communicate.save(str(tts_path))

        # Verifica che il file TTS sia stato creato
        if not tts_path.exists() or tts_path.stat().st_size == 0:
            raise Exception("Generazione audio TTS fallita — file vuoto")

        # Step 5 — Fondi audio + video
        ...
        if result.returncode != 0:
            raise Exception(f"FFmpeg merge fallito: {result.stderr.decode()}")

        update_job(job_id, step=6, step_name="Completato!", progress=100, status="done")
        job["output_file"] = str(output_path)

        for tmp in [audio_path, tts_path]:
            tmp.unlink(missing_ok=True)

    except Exception as e:
        jobs[job_id]["status"]    = "error"
        jobs[job_id]["error"]     = str(e)
        jobs[job_id]["step_name"] = f"Errore: {str(e)}"

def translate_best(text: str, detected_lang: str) -> str:
    """Prova DeepL free, fallback su Google Translate."""
    if detected_lang == "it":
        return text

    # Chunk da 4500 caratteri
    chunks = [text[i:i+4500] for i in range(0, len(text), 4500)]
    translated = []

    for chunk in chunks:
        try:
            # DeepL free — molto più preciso di Google
            t = DeeplTranslator(source="auto", target="it").translate(chunk)
            translated.append(t)
        except Exception:
            try:
                ...
                raise Exception(f"Traduzione fallita: {e2}")
